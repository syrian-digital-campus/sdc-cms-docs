...
Use any root folder name (example: **`SDC-CMS`**). File names may differ slightly (e.g. **`cms-1.24.0.jar`**).
...
├── cms-1.24.0.jar                        # Spring Boot backend (version may differ)
...
| Item | Description |
|------|-------------|
| **`cms-1.24.0.jar`** | Backend application (version number may differ). |
...
- **`INSTALL_DIR`** — folder containing `cms-1.24.0.jar` (e.g. `C:\sdc-install`).
...
| **Global roles** | Version 1.5.0 migrates legacy global `ADMIN` rows to `SUPER_ADMIN`. `SUPER_ADMIN` is the canonical full-system role; stale `ADMIN` reads are accepted only as a compatibility alias. |
...
- **Backend:** JAR file name / version (e.g. `cms-1.24.0.jar`).
...
*Document version: 1.24.0 - package examples updated for the 1.24.0 delivery.*
