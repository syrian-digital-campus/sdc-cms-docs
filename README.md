# Syrian Digital Campus - Course Management System (SDC-CMS)

Comprehensive documentation for the SDC-CMS project.

## 📚 Documentation Index

### Getting Started
- **[00_INTRODUCTION.md](./00_INTRODUCTION.md)** - Project overview, goals, and scope
- **[01_ARCHITECTURE.md](./01_ARCHITECTURE.md)** - System architecture and design decisions
- **[02_SETUP_GUIDE.md](./02_SETUP_GUIDE.md)** - Complete setup instructions
- **[15_BEGINNER_START_AND_DEPLOY.md](./15_BEGINNER_START_AND_DEPLOY.md)** - **Beginner guide:** receive the project, run locally, deploy (start here if you are new)

### Technology Stack
- **[03_TECHNOLOGY_STACK.md](./03_TECHNOLOGY_STACK.md)** - Detailed explanation of all technologies used
- **[04_SPRING_BOOT_GUIDE.md](./04_SPRING_BOOT_GUIDE.md)** - Spring Boot for beginners
- **[05_ANGULAR_GUIDE.md](./05_ANGULAR_GUIDE.md)** - Angular for beginners
- **[06_POSTGRESQL_GUIDE.md](./06_POSTGRESQL_GUIDE.md)** - PostgreSQL basics

### Development
- **[07_DEVELOPMENT_GUIDE.md](./07_DEVELOPMENT_GUIDE.md)** - Development workflow and best practices
- **[08_API_DOCUMENTATION.md](./08_API_DOCUMENTATION.md)** - Complete API reference
- **[09_FRONTEND_GUIDE.md](./09_FRONTEND_GUIDE.md)** - Frontend development guide
- **[10_BACKEND_GUIDE.md](./10_BACKEND_GUIDE.md)** - Backend development guide

### Security & Deployment
- **[11_SECURITY.md](./11_SECURITY.md)** - Security measures and best practices
- **[12_DEPLOYMENT.md](./12_DEPLOYMENT.md)** - Deployment guide

### Additional Resources
- **[13_TROUBLESHOOTING.md](./13_TROUBLESHOOTING.md)** - Common issues and solutions
- **[14_GLOSSARY.md](./14_GLOSSARY.md)** - Terminology and glossary

## 🚀 Quick Start

- **New to the project?** Start with [15_BEGINNER_START_AND_DEPLOY.md](./15_BEGINNER_START_AND_DEPLOY.md).

1. **Read the Introduction**: Start with [00_INTRODUCTION.md](./00_INTRODUCTION.md) to understand what this project is about.

2. **Understand the Architecture**: Read [01_ARCHITECTURE.md](./01_ARCHITECTURE.md) to see how everything fits together.

3. **Set Up Your Environment**: Follow [02_SETUP_GUIDE.md](./02_SETUP_GUIDE.md) to get everything running.

4. **Learn the Technologies**: If you're new to any technology, read the relevant guide:
   - New to Spring Boot? → [04_SPRING_BOOT_GUIDE.md](./04_SPRING_BOOT_GUIDE.md)
   - New to Angular? → [05_ANGULAR_GUIDE.md](./05_ANGULAR_GUIDE.md)
   - New to PostgreSQL? → [06_POSTGRESQL_GUIDE.md](./06_POSTGRESQL_GUIDE.md)

5. **Start Developing**: Follow [07_DEVELOPMENT_GUIDE.md](./07_DEVELOPMENT_GUIDE.md) for development workflows.

## 📖 Reading Order for New Developers

If you're completely new to this project and the technologies used, read in this order:

1. [15_BEGINNER_START_AND_DEPLOY.md](./15_BEGINNER_START_AND_DEPLOY.md) - Hands-on: clone, run, deploy
2. [00_INTRODUCTION.md](./00_INTRODUCTION.md) - What is this project?
3. [01_ARCHITECTURE.md](./01_ARCHITECTURE.md) - How does it work?
4. [02_SETUP_GUIDE.md](./02_SETUP_GUIDE.md) - Get it running locally
5. [03_TECHNOLOGY_STACK.md](./03_TECHNOLOGY_STACK.md) - What technologies are used?
6. Technology-specific guides (if needed):
   - [04_SPRING_BOOT_GUIDE.md](./04_SPRING_BOOT_GUIDE.md)
   - [05_ANGULAR_GUIDE.md](./05_ANGULAR_GUIDE.md)
   - [06_POSTGRESQL_GUIDE.md](./06_POSTGRESQL_GUIDE.md)
7. [07_DEVELOPMENT_GUIDE.md](./07_DEVELOPMENT_GUIDE.md) - Start coding
8. [08_API_DOCUMENTATION.md](./08_API_DOCUMENTATION.md) - Understand the API
9. [11_SECURITY.md](./11_SECURITY.md) - Security considerations

## 🎯 Project Repositories

This project consists of multiple repositories:

- **sdc-cms-backend**: Spring Boot REST API
- **sdc-cms-frontend**: Angular web application
- **sdc-cms-infra**: Docker Compose infrastructure setup
- **sdc-cms-docs**: This documentation repository

## 📝 Contributing

When contributing to the project:

1. Read the relevant documentation first
2. Follow the development guide
3. Ensure your code follows the project's security guidelines
4. Update documentation if you make significant changes

## 🤝 Need Help?

- Check [13_TROUBLESHOOTING.md](./13_TROUBLESHOOTING.md) for common issues
- Review the [14_GLOSSARY.md](./14_GLOSSARY.md) for terminology
- Consult the technology-specific guides if you're learning a new framework

---

**Last Updated**: December 2024  
**Project Status**: Active Development
